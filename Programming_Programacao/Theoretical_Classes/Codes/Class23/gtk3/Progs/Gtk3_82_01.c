...
gchar              *winTitle = "Tamanho do Screen" ;

int                 win_xlen = 400 ;
int                 win_ylen = 200 ;

void
get_window_size (GtkWidget *w, GtkWidget *l1)
{
  char text[64] ;
  ...
  sprintf (text, "O Tamanho do screen é: %d x %d\n", geom.width, geom.height);
...
{
  GtkWidget *window, *vbox, *hbox2, *qmessage, *label ;
  ...
  label = gtk_label_new ("Teste de Tamanho da Janela");
  gtk_box_pack_start (GTK_BOX (vbox), label, FALSE, TRUE, 20);

  hbox2 = gtk_box_new (GTK_ORIENTATION_HORIZONTAL, 0);
  gtk_box_pack_start (GTK_BOX (vbox), hbox2, FALSE, TRUE, 20);

  qmessage = gtk_button_new_with_label ("Tamanho");
  gtk_widget_set_size_request (qmessage, 100, 20);
  g_signal_connect (qmessage, "clicked", G_CALLBACK (get_window_size), label);
