...
{
  GtkWidget *window, *vbox, *hbox1, *hbox2, *bxalign, *byalign;
  GtkWidget *image, *bt1, *label ;
  ...
  image = gtk_image_new_from_icon_name ("mail-unread", GTK_ICON_SIZE_BUTTON);
  bt1 = gtk_button_new ();
  gtk_button_set_image (GTK_BUTTON(bt1), image);
  gtk_widget_set_size_request (bt1, 100, -1);

  frame = gtk_frame_new (NULL);
  gtk_frame_set_label_widget (GTK_FRAME(frame), bt1);
  gtk_frame_set_label_align (GTK_FRAME(frame), xalign, yalign);
