...
gchar   *winTitle    = "Gtk3-Cairo: Rotação ajustada com o rato" ;
glong    win_xlen    = 800 ;
glong    win_ylen    = 600 ;
...
{      
  ...
  cairo_translate (cr, alloc1.width/2, alloc1.height/2);
  cairo_scale (cr, 0.5, 0.5);
  //cairo_scale (cr, scale1xy, scale1xy);
  cairo_rotate (cr, scale1xy * M_PI);

  cairo_move_to (cr, 140, 50);
  cairo_select_font_face(cr, "Courier", CAIRO_FONT_SLANT_NORMAL, CAIRO_FONT_WEIGHT_BOLD);
  cairo_set_font_size (cr, 30);
  cairo_set_source_rgb (cr, 0., 0., 1.);
  strcpy (texto, "Exemplo de GTK+ Cairo - Rotate");
