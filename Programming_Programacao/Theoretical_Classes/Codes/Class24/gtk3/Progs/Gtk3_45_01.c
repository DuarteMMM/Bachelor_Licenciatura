...
{      
  gchar          texto[128] ;
  static double  pos1x = 30., pos1y = 50.;
  ...
  cairo_move_to (cr, 20, alloc1.height - 20);
  strcpy (texto, "Carregar em cada um dos três botões do rato");
  cairo_show_text (cr, texto);
  cairo_stroke (cr);
  ...
  cairo_set_source_rgb (cr, 0.9, 0.4, 0.1);
  cairo_set_line_width (cr, 1.0);
  cairo_rectangle (cr, (gint) pos1x, (gint) pos1y, 80, 50);
  cairo_fill (cr);
  cairo_stroke (cr);

  // Actualizacao da posicao
